...
def ranking(request):
    if request.method == 'POST':
        ...
        ranks = Ranking.objects.filter(platform=Platform.to_i('android'), is_delete=0).order_by('-position')
        return render(request, 'android/rank/ranking.html', {
            'ranks': ranks,
        })
...
def query_ranking(request, query_id):
    try:
        ...
        return HttpResponseRedirect(reverse('android_ranking'))
    return render(request, 'android/rank/update_rank.html', {
        'rank': rank,
    })
