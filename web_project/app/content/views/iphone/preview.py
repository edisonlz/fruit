...
from content.models import Platform, HomeBox, IphoneBoxVideo, IphoneHomeBoxTag
...
def preview(request):
    ...
    for module in modules:
        item = {'title': module.title, 'module_id': module.pk, 'video_count': module.video_count_for_phone,
                'tags': [x.title for x in IphoneHomeBoxTag.objects.filter(box_id=module.id) if x.title != module.title]}
        videos = IphoneBoxVideo.objects.filter(state=1, is_delete=0, box_id=module.id).order_by('-position')
    ...
    return render(request, 'iphone/preview/preview_index.html', {
        'slider': slider,
