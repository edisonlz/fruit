...
import json
from content.models import IphoneChannel, IphoneChannelVideo, VideoType
...
def channel_videos(request):
    ...
    channel_content_type_to_s = IphoneChannel.CONTENT_TYPE[int(channel.content_type)]
    channel_videos = IphoneChannelVideo.objects.filter(channel_id=channel_id,is_delete=False)
    for video in channel_videos:
        video.video_type_name = VideoType.to_s(video.video_type)
    navigation_list = ["频道管理","Iphone频道", channel_content_type_to_s,'视频列表']
    return render(request, 'iphone/channel/channel_videos/iphone_channel_videos.html', {'videos': channel_videos,'channel':channel,
...
def add_channel_video(request):
    if request.method == 'POST':
        ...
        video.state = 1
        video.save()
        return HttpResponseRedirect(reverse('iphone_query_channel_video',args={video.id,}))
    else:
        current_channel_id = request.GET.get('channel_id')
        video_type_list = IphoneChannelVideo.video_types(mock=True)
        return render(request, 'iphone/channel/channel_videos/iphone_add_video.html', {
...
def query_channel_video(request,video_id):
    video = get_object_or_404(IphoneChannelVideo, pk=video_id)
    ...
    if video_type_name == 'game_details' or video_type_name == 'game_download':
        ...
        return render(request, 'iphone/channel/channel_videos/update_game_details.html', {'video': video,
                        'channel_id': channel.id, 'game': game})
    elif video_type_name in ('video', 'show', 'playlist','url'):
        if video.attached_game_type:
            game = IosGame.objects.get(id=video.game_id)
            return render(request, 'iphone/channel/channel_videos/update_video_with_game_details.html', {'video': video,
                                'game': game, 'channel_id': channel.id,})
        else:
            return render(request, 'iphone/channel/channel_videos/update_video_form_fields.html', {'video': video,'channel_id': channel.id})
...
def update_channel_video(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if video_id:
                video = IphoneChannelVideo.objects.get(pk=video_id)
            ...
            print e
        return HttpResponseRedirect(reverse('iphone_channel_videos') + "?channel_id=" + channel_id)
...
def update_channel_video_status(request):
    if request.method == 'POST':
        ...
        if video_ids and value:
            try:
                hbs = IphoneChannelVideo.objects.extra(where=['id IN (%s)' % video_ids]).update(state=int(value))
...
def delete_channel_video(request,video_id):
    channel_id = request.GET.get("channel_id", '')
    try:
        IphoneChannelVideo.objects.filter(pk=video_id).update(is_delete=1)
    except (IphoneChannelVideo.DoesNotExist):
        pass
    return HttpResponseRedirect(reverse('iphone_channel_videos') + "?channel_id=" + channel_id)
