...
import json
from content.models import HomeBox, HomeBoxTag, IphoneHomeBoxTag, IphoneHomeBoxTag, IphoneSubChannel, IphoneChannel, \
    TagType, IosGame, IosGame, Platform
...
def add_module_tag(request):
    ...
    box_tag = IphoneHomeBoxTag()
    old_tags = IphoneHomeBoxTag.objects.filter(box_id=box_id, tag_type="normal", is_delete=0)
    ...
    box_tag.save()
    return render(request, "iphone/main_page/module_manage/module_tag/tag.html", {"tag": box_tag})
...
def delete_module_tag(request):
    ...
    if tag_id:
        box_tag = IphoneHomeBoxTag.objects.get(pk=tag_id)
...
def query_module_tag(request, tag_id):
    tag_id = int(tag_id)
    tag = IphoneHomeBoxTag.objects.get(pk=tag_id)
    box_id = tag.box_id
    ...
    return render(request, 'iphone/main_page/module_manage/module_tag/update_tag.html', {
...
def tag_sub_channel_options(request):
    ...
    if (len(sub_channel_id) == 0 or int(sub_channel_id) == 0) and len(channel_id) == 0:
        ...
        if sub_channel_id and len(sub_channel_id) > 0 and int(sub_channel_id) > 0:
            sub_channel = IphoneSubChannel.objects.get(pk=int(sub_channel_id))
        if sub_channel:
            channel = sub_channel.channel
        else:
            channel = IphoneChannel.objects.get(pk=request.POST.get("channel_entity_id"))
        sub_channels = IphoneSubChannel.objects.filter(channel_id=channel.id, is_delete=0).order_by("position")
        options = ""
